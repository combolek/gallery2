...
RotatePhotoBlock.prototype.submit = function(RotatePhotoBlock, target, self) {
  ...
  var status = YAHOO.util.Dom.get(GalleryUtilities.elementId("status", "RotatePhotoBlock",
    self.templateId));
  var context = YAHOO.util.Dom.get(RotatePhotoBlock.id);
  var position = YAHOO.util.Dom.getXY(context);
  position[0] += (context.offsetWidth - status.offsetWidth) / 2;
  position[1] += (context.offsetHeight - status.offsetHeight) / 2;
  YAHOO.util.Dom.setXY(GalleryUtilities.elementId("status", "RotatePhotoBlock", self.templateId),
    position);
